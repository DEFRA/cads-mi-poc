...
> [!WARNING]
> The `cads-mi-poc` is not a production ready application, it is a tool for prototyping. It is not
...
The refreshed GOV.UK brand is available and turned on by default in the `cads-mi-poc`. To turn it
...
```bash
docker build --target development --no-cache --tag cads-mi-poc:development .
```

Run:

```bash
docker run -e PORT=3000 -p 3000:3000 cads-mi-poc:development
```
...
```bash
docker build --no-cache --tag cads-mi-poc .
```
...
```bash
docker run -e PASSWORD=beepBoopBeep -e PORT=3000 -p 3000:3000 cads-mi-poc
```
...
```bash
docker run -it --entrypoint /bin/ash cads-mi-poc
```
